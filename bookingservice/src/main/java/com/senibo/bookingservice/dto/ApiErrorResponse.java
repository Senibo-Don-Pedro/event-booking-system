...
public record ApiErrorResponse(
        @Schema(description = "Indicates failure", example = "false") boolean success,

        @Schema(description = "Error message", example = "Invalid email or password") String message,

        @Schema(description = "Timestamp of the error", example = "2024-12-06T10:30:00") LocalDateTime timestamp,

        @Schema(description = "List of detailed error messages (for validation errors)", example = "[\"email: Email must be valid\", \"password: Password is required\"]") List<String> errors) {
